    ...
    open fun printNode(node: Node?, nestSize: Int): PrimeDoc {
        ...
        val followingCode =
                when (node) {
                    ...
                    is While -> {
                        val innerNode = node.getNode()
                        val followingCode = if (innerNode!!.getNextNode() != null) printNode(innerNode!!.getNextNode(), nestSize) else nil()
                        line() + text("while (!(") + printExpression(innerNode!!.getCondition(), nestSize) + text(")) {") + nest(nestSize, printNode(innerNode!!.getListOfTails()!!.get(0), nestSize)) / text("}") + followingCode
                    }
                    else ->
