...
fun printExpression(expression: Expression?, nestSize: Int): PrimeDoc =
    when (expression) {
        ...
        is com.sdc.ast.expressions.Invocation -> {
            var funName = group(text(/*"this." + */expression.getFunction() + "("))
...
fun printStatement(statement: Statement, nestSize: Int): PrimeDoc =
    when (statement) {
        is Invocation -> {
            var funName = group(text(/*"this." + */statement.getFunction() + "("))
...
fun printJavaClass(javaClass: JSClass): PrimeDoc {
    ...
    */
    ...
    var javaClassCode = group(text("function " + javaClass.getName() + "() {"))
    /*
    for (classField in javaClass.getFields()!!.toArray())
        javaClassCode = group(
                javaClassCode
                + nest(javaClass.getNestSize(), line() + printClassField(classField as JSClassField))
        )
      */
    for (classMethod in javaClass.getMethods()!!.toArray())
        javaClassCode = group(
                javaClassCode
                + nest(javaClass.getNestSize(), line() + printClassMethod(classMethod as JSClassMethod))
        )

    return group(javaClassCode / text("}"))
}

fun printClassMethod(classMethod: JSClassMethod): PrimeDoc {
    var declaration = group(text(""))
    /*
    val generi      csDeclaration = classMethod.getGenericDeclaration()
    if (!genericsDeclaration!!.isEmpty()) {
        declaration = group(declaration + text("<"))
        var oneType = true
        for (genericType in genericsDeclaration) {
    ...
    declaration = group(declaration + text(/*   "this." + */classMethod.getName() + "("))
...
fun printClassField(classField: JSClassField): PrimeDoc =
